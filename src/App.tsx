...
import { Home, Lightbulb, GraduationCap, LayoutGrid, CheckSquare } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex font-sans text-gray-200 overflow-hidden">
      ...
      <aside className="w-64 bg-[#0a0a0a] border-r border-gray-800 flex flex-col fixed h-full z-10">
        ...
        <nav className="flex-1 px-4 space-y-2">
          ...
          <div className="pt-6 pb-2">
            <span className="px-4 text-[10px] font-bold uppercase tracking-widest text-gray-500">Specialization</span>
          </div>
      ...
      <main className="flex-1 ml-64 p-8 overflow-y-auto h-screen">
         {activeTab === 'dashboard' && <Dashboard progress={progress} setTab={setActiveTab} />}
         {activeTab === 'test' && <CompetencyTest progress={progress} updateProgress={setProgress} />}
         {activeTab === 'concepts' && <Concepts progress={progress} updateProgress={setProgress} />}
         {activeTab === 'courses' && <Courses progress={progress} updateProgress={setProgress} />}
         {activeTab === 'claude' && <ClaudeArchitect progress={progress} updateProgress={setProgress} />}
      </main>
